...
<Aside type="note">
    这是一份基于 Google 代码规范的格式化标准。查阅 [Clang-Format
    选项文档](https://clang.llvm.org/docs/ClangFormatStyleOptions.html) 了解更多。
</Aside>
...
2. **配置工作空间编译参数**
    ...
    <Tabs>
        <TabItem label="catkin build">
            ```bash catkin config --cmake-args -DCMAKE_EXPORT_COMPILE_COMMANDS=ON catkin build ```
        </TabItem>
        <TabItem label="catkin_make">
            ```bash catkin_make -DCMAKE_EXPORT_COMPILE_COMMANDS=1 ```
        </TabItem>
    </Tabs>
...
3. **验证配置**
    ...
    <Aside type="tip">
        如果文件分散在各包名目录下，可使用
        [合并脚本](https://github.com/catkin/catkin_tools/issues/551#issuecomment-553521463)
        进行合并。
    </Aside>
